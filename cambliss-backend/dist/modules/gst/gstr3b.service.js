...
exports.generateGSTR3BReport = exports.generateGSTR3BJSON = exports.generateGSTR3BSummary = void 0;
 ...
 */
const generateGSTR3BSummary = (organizationId, month, year) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    ...
    const salesInvoices = yield prisma_1.default.invoice.findMany({
        where: {
            organizationId,
            status: "ISSUED",
            issuedAt: {
    ...
    try {
        ...
        const bills = yield ((_a = prisma_1.default.bill) === null || _a === void 0 ? void 0 : _a.findMany({
            where: {
                organizationId,
                status: "APPROVED",
                date: {
...
exports.generateGSTR3BSummary = generateGSTR3BSummary;
/**
 * Generate JSON for GSTR-3B filing
 */
const generateGSTR3BJSON = (summary) => {
    return {
...
exports.generateGSTR3BJSON = generateGSTR3BJSON;
/**
 * Generate human-readable summary report
 */
const generateGSTR3BReport = (summary) => {
    const lines = [];
    lines.push("═══════════════════════════════════════════════════════");
    ...
    lines.push(`   ────────────────────────────────────────────`);
    lines.push(`   TOTAL OUTPUT GST: ₹${summary.outputGST.total.toLocaleString("en-IN", { minimumFractionDigits: 2 })}`);
    lines.push("");
    lines.push("───────────────────────────────────────────────────────");
    ...
    lines.push(`   ────────────────────────────────────────────`);
    lines.push(`   TOTAL INPUT GST: ₹${summary.inputGST.total.toLocaleString("en-IN", { minimumFractionDigits: 2 })}`);
    lines.push("");
    lines.push("═══════════════════════════════════════════════════════");
    ...
    lines.push(`   ────────────────────────────────────────────`);
    lines.push(`   TOTAL PAYABLE: ₹${summary.netPayable.total.toLocaleString("en-IN", { minimumFractionDigits: 2 })}`);
    lines.push("");
    if (summary.itcCarryForward.total > 0) {
        lines.push("───────────────────────────────────────────────────────");
        lines.push("4. ITC CARRY FORWARD (Credit Available)");
        lines.push("───────────────────────────────────────────────────────");
        lines.push(`   CGST: ₹${summary.itcCarryForward.cgst.toLocaleString("en-IN", { minimumFractionDigits: 2 })}`);
        lines.push(`   SGST: ₹${summary.itcCarryForward.sgst.toLocaleString("en-IN", { minimumFractionDigits: 2 })}`);
        lines.push(`   IGST: ₹${summary.itcCarryForward.igst.toLocaleString("en-IN", { minimumFractionDigits: 2 })}`);
        lines.push(`   ────────────────────────────────────────────`);
        lines.push(`   TOTAL ITC CARRY FORWARD: ₹${summary.itcCarryForward.total.toLocaleString("en-IN", { minimumFractionDigits: 2 })}`);
        lines.push("");
    }
    lines.push("───────────────────────────────────────────────────────");
    ...
    lines.push(`   Total Sales Value: ₹${summary.metadata.totalOutputValue.toLocaleString("en-IN", { minimumFractionDigits: 2 })}`);
    lines.push(`   Total Purchase Value: ₹${summary.metadata.totalInputValue.toLocaleString("en-IN", { minimumFractionDigits: 2 })}`);
    lines.push("");
    lines.push("═══════════════════════════════════════════════════════");
...
exports.generateGSTR3BReport = generateGSTR3BReport;

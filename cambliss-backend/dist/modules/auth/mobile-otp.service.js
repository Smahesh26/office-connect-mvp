...
exports.isMobileOtpEnabled = exports.consumeVerifiedRegisterOtp = exports.verifyRegisterOtp = exports.sendRegisterOtp = exports.MobileOtpError = void 0;
const crypto_1 = __importDefault(require("crypto"));
class MobileOtpError extends Error {
...
exports.MobileOtpError = MobileOtpError;
...
const OTP_RETURN_IN_RESPONSE = (process.env.MOBILE_OTP_RETURN_IN_RESPONSE || (process.env.NODE_ENV !== "production" ? "true" : "false")).toLowerCase() === "true";
const otpSessions = new Map();
const latestRequestByPhoneAndPurpose = new Map();
const toPhoneKey = (phone, purpose) => `${purpose}:${phone}`;
const normalizePhone = (phone) => phone.trim().replace(/\s+/g, "");
const isValidPhone = (phone) => /^\+?[1-9]\d{9,14}$/.test(phone);
const hashOtp = (phone, otp) => {
    return crypto_1.default.createHash("sha256").update(`${phone}:${otp}:${OTP_PEPPER}`).digest("hex");
};
...
const sendViaConsole = (phone, otp) => __awaiter(void 0, void 0, void 0, function* () {
    console.log(`[MOBILE_OTP][CONSOLE] OTP ${otp} for ${phone}`);
});
const sendViaWebhook = (phone, otp) => __awaiter(void 0, void 0, void 0, function* () {
...
const dispatchOtp = (phone, otp) => __awaiter(void 0, void 0, void 0, function* () {
    if (!OTP_ENABLED) {
        return;
    }
    if (OTP_PROVIDER === "WEBHOOK") {
        yield sendViaWebhook(phone, otp);
        return;
    }
    yield sendViaConsole(phone, otp);
});
const sendRegisterOtp = (rawPhone) => __awaiter(void 0, void 0, void 0, function* () {
    cleanupExpired();
    ...
    yield dispatchOtp(phone, otp);
    return Object.assign({ requestId, expiresInSeconds: OTP_EXPIRY_SECONDS, resendAfterSeconds: OTP_RESEND_SECONDS }, (OTP_RETURN_IN_RESPONSE ? { otp } : {}));
});
exports.sendRegisterOtp = sendRegisterOtp;
const verifyRegisterOtp = (params) => {
    var _a, _b;
    cleanupExpired();
    const phone = normalizePhone(params.phone || "");
    const requestId = (_a = params.requestId) === null || _a === void 0 ? void 0 : _a.trim();
    const otp = (_b = params.otp) === null || _b === void 0 ? void 0 : _b.trim();
...
const consumeVerifiedRegisterOtp = (params) => {
    var _a;
    if (!OTP_ENABLED) {
        return;
    }
...
exports.consumeVerifiedRegisterOtp = consumeVerifiedRegisterOtp;
const isMobileOtpEnabled = () => OTP_ENABLED;
exports.isMobileOtpEnabled = isMobileOtpEnabled;

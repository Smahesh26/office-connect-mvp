...
export default function AdminOrderHistoryPage() {
  ...
  const [orders, setOrders] = useState<AdminOrderHistoryItem[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      ...
      try {
        ...
        const data = await response.json() as AdminOrderHistoryItem[];
  ...
  const downloadInvoice = async (paymentId: string) => {
    ...
    const response = await fetch(`/api/admin/order-history/${paymentId}/invoice`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
  ...
  return (
    <WorkspaceShell>
      <div className="relative mt-5 overflow-hidden rounded-2xl border border-zinc-200 bg-gradient-to-br from-white via-zinc-50 to-zinc-100 p-6 shadow-[0_24px_56px_-30px_rgba(0,0,0,0.85)] ring-1 ring-white/80">
        ...
        <h1 className="text-2xl font-semibold tracking-tight text-zinc-900">Admin Order History</h1>
        <p className="mt-1 text-sm text-zinc-600">View all client orders and download invoices.</p>

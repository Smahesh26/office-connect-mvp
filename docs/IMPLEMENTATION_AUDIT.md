...
| Domain / Feature | Feature Name | Frontend Location | Backend / API Location | DB Models Involved | External Dependencies | Current Status | What Actually Works | What is Mocked / Incomplete | Security / Blocker Concerns | Recommended Next Action |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
...
| **Auth & Access** | Seller Portal Authentication | `app/vendor-dashboard` | `/api/auth/*` | `Store`, `StoreMember`, `User` | None | ✅ FIXED (P0) | Next.js Server Middleware (`middleware.ts`) blocks unauthenticated requests & redirects to `/login` | Full seller onboarding forms are mocked | Route guards active | Derives `sellerId` server-side from JWT claims |
| **Auth & Access** | Super-Admin RBAC | `app/admin-dashboard` | `/api/admin/*` | `User`, `Role`, `OrganizationUser` | None | ✅ FIXED (P0) | Next.js Server Middleware (`middleware.ts`) blocks unauthenticated requests & redirects to `/login` | SaaS admin API exists; UI buttons mutate local state | Route guards active | Protect route with `SUPER_ADMIN` server session check |

...
| Location in Code | Exact Claim Displayed | Claim Type | Reality / Evidence Status | Risk Severity | Required Action & Status |
| :--- | :--- | :--- | :--- | :--- | :--- |
| `StorefrontFooter.tsx:129` | `"ISO 27001 Certified • PCI-DSS Level 1"` | Security & Compliance Certification | 🚫 **UNVERIFIED**: No ISO 27001 certificate or Level 1 PCI-DSS Attestation of Compliance (AoC) on file. | 🔴 **HIGH LEGAL RISK** | ✅ **SANITIZED**: Replaced with generic `"Encrypted 256-Bit SSL • TLS 1.3 Security"`. |
| `StorefrontFooter.tsx:75`, `StorefrontHero.tsx:14`, `ProductBuyBox.tsx:156` | `"100% Escrow Protection: Funds held securely until verified delivery OTP scan"` | Financial & Banking Escrow Guarantee | 🚫 **MOCK INFRASTRUCTURE**: No legal tripartite escrow agreement with RBI-approved escrow bank (e.g. HDFC/ICICI) or live Razorpay Route/Stripe Connect marketplace escrow account. | 🔴 **HIGH REGULATORY RISK** | ✅ **SANITIZED**: Replaced with neutral `"Secure Payment Processing"` / `"Encrypted Payment Protection"`. |
| `StorefrontFooter.tsx:88`, `StorefrontAnnouncementBar.tsx:18` | `"Express 48-Hour Dispatch Across All Verified Stores"` | Logistics SLA Guarantee | 🟡 **UNENFORCED**: No automated carrier webhook SLA enforcement or courier dispatch integration active. | 🟡 **MEDIUM (OPERATIONAL)** | ✅ **SANITIZED**: Replaced with `"Express Dispatch Options"`. |
| `StorefrontFeaturedStores.tsx:63`, `SellerHeroHeader.tsx:48` | `"Verified Genuine Sellers • 5-Stage KYB Vetted"` | Merchant Credential Verification | 🟡 **MOCK WORKFLOW**: The 5-stage onboarding UI exists, but approval is simulated in React state without actual government API (MCA/GSTIN/PAN) verification. | 🟡 **MEDIUM (TRUST)** | ✅ **SANITIZED**: Replaced with `"Merchant Verification Checklist"`. |
| `StorefrontFeaturedBrands.tsx:22`, `brand/[slug]/page.tsx` | `"Sony Direct 👑 Flagship Store"`, `"Apple Direct"`, `"Keychron Official"` | Brand Authorization & Trademarks | 🚫 **DEMO PLACEHOLDER**: No formal distributor or brand authorization agreement with Sony India, Apple Inc., or Keychron. | 🔴 **HIGH TRADEMARK RISK** | ✅ **SANITIZED**: Replaced trademarked brands in demo fixtures with neutral brands (*AeroTech*, *Lumina Keyboards*, *UrbanStyle*). |
